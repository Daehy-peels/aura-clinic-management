        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
          <div className="animate-in fade-in slide-in-from-bottom duration-700">
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-[10px] font-black text-rose-400 uppercase tracking-[0.4em]">Systems Operational</span>
            </div>
            ...
            <p className="text-slate-400 font-medium mt-4 tracking-wide">
              {format(currentTime, 'EEEE, MMMM do')} — {format(currentTime, 'yyyy')}
            </p>
          </div>
          ...
          <div className="bg-white border border-rose-100/50 px-10 py-6 rounded-[2.5rem] shadow-[0_15px_40px_rgba(255,192,203,0.1)] text-right">
            <p className="text-[9px] font-black text-rose-300 uppercase tracking-[0.4em] mb-2">Live Clinic Time</p>
            <p className="text-4xl font-light text-slate-700 tracking-tight">
              {format(currentTime, 'hh:mm:ss')} <span className="text-lg font-bold text-rose-400">{format(currentTime, 'aa')}</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 space-y-8">
            <div className="flex items-center justify-between px-2">
              <h2 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em]">Next In Treatment</h2>
              <Link href="/dashboard/appointments" className="text-[10px] font-black text-rose-400 hover:text-rose-600 transition-colors uppercase tracking-widest border-b border-rose-100 pb-1">Full Schedule</Link>
            </div>

            <div className="bg-white border border-slate-100 rounded-[4rem] p-4 shadow-[0_20px_60px_rgba(0,0,0,0.02)]">
              {loading ? (
                <div className="p-20 text-center animate-pulse text-rose-200 font-serif italic text-xl">Accessing schedule...</div>
              ) : upcoming.length > 0 ? (
                <div className="space-y-3">
                  {upcoming.map((apt) => (
                    <div key={apt.id} className="group flex items-center justify-between p-10 hover:bg-rose-50/30 rounded-[3rem] transition-all duration-500 cursor-pointer">
                      <div className="flex items-center gap-12">
                        <div className="text-center">
                          <p className="text-2xl font-light text-slate-800 tracking-tighter">
                            {format(parseISO(apt.scheduled_at), 'hh:mm')}
                          </p>
                          <p className="text-[9px] font-black text-rose-400 uppercase">{format(parseISO(apt.scheduled_at), 'aa')}</p>
                        </div>
                        <div className="h-12 w-[1px] bg-slate-100 group-hover:bg-rose-200 transition-colors" />
                        <div>
                          <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-2">Patient Profile</p>
                          <p className="text-xl font-bold text-slate-800 mb-1">{apt.patients?.first_name} {apt.patients?.last_name}</p>
                          <p className="text-lg font-serif italic text-rose-500 opacity-80 group-hover:opacity-100">{apt.treatment_type}</p>
                        </div>
                      </div>
                ...
                <div className="p-24 text-center text-slate-300 italic font-serif text-xl">The afternoon is clear.</div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-4 space-y-10">
            <div className="bg-slate-900 rounded-[3.5rem] p-12 text-white shadow-2xl relative overflow-hidden group">
              ...
              <h2 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.4em] mb-10">Administrative</h2>
            ...
            <div className="bg-rose-50 border border-rose-100 rounded-[3.5rem] p-12 relative overflow-hidden">
               <p className="text-[10px] font-black text-rose-400 uppercase tracking-[0.4em] mb-8">Clinical Vision</p>
               <p className="text-2xl font-light text-slate-700 leading-tight italic font-serif">
                "Precision is the <span className="text-rose-500">signature</span> of clinical excellence."
               </p>
               <div className="mt-10 pt-8 border-t border-rose-200/40">
                  <p className="text-[9px] font-bold text-rose-300 uppercase tracking-widest">Protocol V.2026.04</p>
               </div>
            </div>
          </div>
        </div>
...
function StatCard({ title, value, subtitle, icon, color = "text-slate-800" }: any) {
  return (
    <div className="group bg-white border border-slate-100 rounded-[3.5rem] p-12 shadow-[0_10px_40px_rgba(0,0,0,0.02)] hover:shadow-rose-100 hover:-translate-y-1 transition-all duration-500">
      <div className="flex justify-between items-start mb-8">
        ...
        <div className="text-rose-400 group-hover:scale-110 transition-transform duration-500">{icon}</div>
      </div>
      <p className={`text-7xl font-light tracking-tighter ${color}`}>{value}</p>
      <p className="text-[10px] text-slate-400 mt-6 font-black uppercase tracking-[0.2em]">{subtitle}</p>
    </div>
  );
}
...
function QuickActionLink({ href, label }: { href: string; label: string }) {
  return (
    <Link href={href} className="flex items-center justify-between w-full p-6 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black text-white uppercase tracking-[0.2em] hover:bg-rose-500 hover:border-rose-500 transition-all group">
      {label}
