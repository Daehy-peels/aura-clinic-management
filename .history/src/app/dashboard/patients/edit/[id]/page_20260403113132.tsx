...
export default function EditPatientPage() {
  ...
  if (loading) return <div className="min-h-screen bg-[#FFF5F5] flex items-center justify-center font-serif italic text-rose-400">Opening file...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F5] p-8 md:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Header */}
        <div className="mb-10">
          <Link href={`/dashboard/patients/${id}`} className="text-[10px] font-bold uppercase tracking-[0.2em] text-rose-400 hover:text-rose-600 transition-colors">
            ← Cancel and Return
          </Link>
          <h1 className="text-4xl font-light text-gray-800 mt-4 tracking-tight">
            Update <span className="font-serif italic text-rose-500">Patient File</span>
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white/80 backdrop-blur-xl p-8 md:p-12 rounded-[3rem] shadow-[0_20px_50px_rgba(255,192,203,0.08)] border border-white">
            <h2 className="text-[10px] font-bold text-rose-400 uppercase tracking-widest mb-10">Demographics & Contact</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
              <InputGroup label="First Name" value={formData.first_name} 
                onChange={(val) => setFormData({...formData, first_name: val})} />
              
              <InputGroup label="Last Name" value={formData.last_name} 
                onChange={(val) => setFormData({...formData, last_name: val})} />
              
              <InputGroup label="Email Address" type="email" value={formData.email} 
              ...
              <div className="md:col-span-2">
                <label className="text-[10px] font-bold text-rose-400 uppercase tracking-widest ml-1">Date of Birth</label>
                <input 
                  type="date" 
          ...
          <div className="bg-white/80 backdrop-blur-xl p-8 md:p-12 rounded-[3rem] shadow-[0_20px_50px_rgba(255,192,203,0.05)] border border-white">
            <h2 className="text-[10px] font-bold text-rose-400 uppercase tracking-widest mb-6">Medical History & Allergies</h2>
            <textarea 
              rows={6}
...
function InputGroup({ label, value, onChange, type = "text" }: any) {
  return (
    <div className="group">
      <label className="text-[10px] font-bold text-rose-400 uppercase tracking-widest ml-1">{label}</label>
      <input 
        type={type} 
        value={value}
