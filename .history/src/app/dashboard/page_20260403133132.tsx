        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
          <div className="space-y-2">
            <div className="flex items-center gap-3 mb-4">
              <span className="px-3 py-1 bg-rose-100 text-rose-500 text-[10px] font-black uppercase tracking-[0.2em] rounded-full">
                Systems Active
              </span>
            </div>
          ...
          <div className="bg-white/40 backdrop-blur-md px-8 py-4 rounded-[2rem] border border-white shadow-sm text-right min-w-[200px]">
            <p className="text-[10px] font-black text-rose-300 uppercase tracking-[0.3em]">
              Live Clinic Time
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between px-4">
              <h2 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.3em]">
                Next in Treatment
              </h2>
              <Link
                href="/dashboard/appointments"
                className="text-[10px] font-bold text-gray-400 hover:text-rose-500 transition-colors uppercase tracking-widest"
              >
                Full Schedule →
              </Link>
            </div>
            ...
            <div className="bg-white/60 backdrop-blur-xl rounded-[3.5rem] p-4 border border-white shadow-[0_30px_60px_rgba(255,192,203,0.08)]">
              {loading ? (
                <div className="p-12 text-center animate-pulse text-rose-200 font-serif italic">
                  Loading agenda...
                </div>
              ) : upcoming.length > 0 ? (
                <div className="space-y-2">
                  {upcoming.map((apt) => (
                    <div
                      key={apt.id}
                      className="group flex items-center justify-between p-8 bg-transparent hover:bg-white rounded-[2.5rem] transition-all duration-500 cursor-pointer"
                    >
                      <div className="flex items-center gap-8">
                        ...
                        <div>
                          ...
                          <p className="text-xl font-serif italic text-rose-500 opacity-80 group-hover:opacity-100">
                            {apt.treatment_type}
                          </p>
                        </div>
                      </div>
          ...
          <div className="space-y-8">
            <div className="bg-white/80 backdrop-blur-xl rounded-[3rem] p-10 border border-white shadow-xl shadow-rose-100/20">
              <h2 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.3em] mb-8">
                Navigation
              </h2>
            ...
            <div className="bg-rose-500 rounded-[3.5rem] p-12 text-white shadow-2xl shadow-rose-200 relative overflow-hidden group hover:-translate-y-2 transition-all duration-500">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-60 mb-8">
                Clinical Note
              </p>
              <p className="text-2xl font-light leading-snug">
                "Precision is the{" "}
                <span className="italic font-serif">signature</span> of
                excellence."
              </p>
  ...
  return (
    <div className="group bg-white/70 backdrop-blur-xl rounded-[3rem] p-10 border border-white shadow-[0_20px_40px_rgba(255,192,203,0.04)] hover:shadow-rose-100 hover:-translate-y-1 transition-all duration-500">
      <div className="flex justify-between items-start mb-6">
        <h3 className="text-[10px] font-black text-rose-300 uppercase tracking-[0.3em]">
          {title}
        </h3>
      ...
      <p className="text-[10px] text-gray-400 mt-4 font-bold uppercase tracking-[0.2em]">
        {subtitle}
      </p>
    </div>
  );
...
function QuickActionLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="flex items-center justify-between w-full px-8 py-5 bg-rose-50/50 border border-rose-100 rounded-2xl text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] hover:bg-rose-500 hover:text-white transition-all group"
    >
      {label}
