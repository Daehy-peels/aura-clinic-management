...
export default function AppointmentsPage() {
  ...
  // Helper to display "Wall Clock" time exactly as stored in DB
  const formatWallTime = (isoString: string) => {
    if (!isoString) return { time: "--:--", day: "No Date" };

    try {
      // 1. Take ONLY the first 19 characters (YYYY-MM-DDTHH:mm:ss)
      // This ignores the +00:00 or Z offset entirely.
      const wallClockPart = isoString.substring(0, 19);
      const date = new Date(wallClockPart);

      if (isNaN(date.getTime())) {
        return { time: "--:--", day: "Invalid Date" };
      }
      ...
      console.error("Date error:", isoString, err);
      return { time: "--:--", day: "Format Error" };
    }
  };
              ...
              return (
                ...
                >
                  ...
                  <div className="text-center md:border-r border-gray-100 md:pr-10 min-w-[120px]">
                    <div className="text-3xl font-light tracking-tighter text-gray-800">
                      {time}
                    </div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-rose-300 mt-1">
                      {day}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-1 text-center md:text-left">
                    ...
                    {apt.notes && (
                      <div className="flex items-start gap-2 mt-4 text-gray-400">
                        ...
                        <p className="text-xs italic font-light line-clamp-1">
                          {apt.notes}
                        </p>
                      </div>
                    )}
                  ...
                  {/* ACTIONS */}
                  <div className="flex items-center gap-1 bg-white/80 p-2 rounded-[2rem] border border-white shadow-sm">
                    {/* Complete/Reopen Status */}
                    <button
                      onClick={() =>
                        updateStatus(apt.id, isComp ? "scheduled" : "completed")
                      }
                      ...
                      title={isComp ? "Reopen Appointment" : "Mark Complete"}
                    >
                      <CheckIcon />
                    </button>
                    ...
                    <Link
                      ...
                      className="p-3 text-gray-300 hover:text-rose-500 hover:bg-rose-50 rounded-2xl transition-all active:scale-90"
                    ...
                    {!isCan && (
                      <button
                        ...
                        className="p-3 text-gray-300 hover:text-orange-400 hover:bg-orange-50 transition-all active:scale-90"
                    ...
                    <button
                      ...
                      className="p-3 text-gray-300 hover:text-red-500 hover:bg-red-50 transition-all active:scale-90"
                      title="Delete Record"
                    >
                      <TrashIcon />
                    </button>
