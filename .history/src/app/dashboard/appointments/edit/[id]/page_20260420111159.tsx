...
export default function EditAppointmentPage() {
  ...
  useEffect(() => {
    async function checkAvailability() {
      const { data } = await supabase
        .from("appointments")
        .select(
          "scheduled_at, ended_at, treatment_type, patients(first_name, last_name)",
        )
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    const ended_at = format(
      addMinutes(parseISO(formData.scheduled_at), duration),
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-[#FFF5F5] flex items-center justify-center font-serif text-rose-400 animate-pulse">
        Loading...
      </div>
    );
    ...
    <div className="min-h-screen bg-[#FFF5F5] p-6 md:p-12 text-gray-800">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-400 text-[10px] uppercase tracking-widest font-bold mt-2 bg-white/50 w-fit px-3 py-1 rounded-full border border-white">
            Editing: {patientName}
          </p>
        </div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {Object.keys(TREATMENTS).map((t) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, treatment_type: t })
                      }
                    ...
                    >
                      {t}
                      <span className="block text-[9px] opacity-70 font-medium">
                        {TREATMENTS[t]} Minutes
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {timeSlots.map((time) => {
                  ...
                  const isInSelectedRange =
                    currentSlotTime >= start && currentSlotTime < end;
                  ...
                  return (
                    <div key={time} className="group relative">
                      ...
                      >
                        {time}
                        {occupant && (
                          <span className="block text-[7px] mt-1 uppercase opacity-60">
                            Reserved
                          </span>
                        )}
                        {!occupant && disabled && !isInSelectedRange && (
                          <span className="block text-[7px] mt-1 uppercase text-orange-300">
                            Overlap
                          </span>
                        )}
                      </button>
                      ...
                      {occupant && (
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-3 bg-gray-900 text-white rounded-xl text-[10px] opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50 shadow-xl">
                          <p className="font-black text-rose-400 uppercase tracking-tighter mb-1 border-b border-white/10 pb-1">
                            Occupied By
                          </p>
                          <p className="font-serif italic text-sm">
                            {occupant.patients?.first_name}{" "}
                            {occupant.patients?.last_name}
                          </p>
                          <p className="mt-1 text-gray-400 uppercase font-bold tracking-widest">
                            {occupant.treatment_type}
                          </p>
