...
export default function EditAppointmentPage() {
  ...
  const getOccupant = (timeStr: string) => {
    ...
    for (const apt of dayAppointments) {
      ...
      if (slotTime >= start && slotTime < end) return apt.patients?.first_name || "Occupied";
    }
    return null;
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (hasCollision) {
      alert("Conflict detected: This duration overlaps with another appointment.");
      return;
    }
  ...
  if (loading) return <div className="p-20 text-center font-serif italic text-rose-500">Syncing Records...</div>;
    ...
    <div className="min-h-screen bg-[#FFFDFD] p-6 md:p-12">
      <div className="max-w-7xl mx-auto">
        <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <Link href="/dashboard/appointments" className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-rose-500">← Back</Link>
            <h1 className="text-6xl font-light tracking-tighter mt-2">Edit <span className="font-serif italic text-rose-500">Session</span></h1>
          </div>
          <div className="bg-white border border-slate-100 p-6 px-10 rounded-[2rem] shadow-sm">
            <p className="text-[9px] font-black text-rose-300 uppercase mb-1">Patient</p>
        ...
        <form onSubmit={handleUpdate} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-3 gap-2 bg-slate-50 p-1 rounded-2xl">
                {['scheduled', 'completed', 'cancelled'].map(s => (
                  <button key={s} type="button" onClick={() => setFormData({...formData, status: s})} className={`py-2 text-[8px] font-black uppercase rounded-xl transition-all ${formData.status === s ? 'bg-white text-rose-500 shadow-sm' : 'text-slate-400'}`}>{s}</button>
                ))}
              </div>

              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">Duration</label>
                <select className="w-full p-4 rounded-2xl bg-slate-50 border-none" value={formData.duration} onChange={(e) => setFormData({...formData, duration: parseInt(e.target.value)})}>
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">Clinical Notes</label>
                <textarea rows={4} className="w-full p-4 rounded-2xl bg-slate-50 border-none font-serif italic" value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} placeholder="Notes..." />
              </div>
            </div>
          ...
          <div className="lg:col-span-8 bg-white p-10 rounded-[4rem] border border-slate-100 shadow-sm">
            <div className="flex justify-between items-center mb-10">
              <h3 className="text-2xl font-light">Clinic <span className="italic font-serif text-rose-500">Timeline</span></h3>
              <input type="date" className="bg-rose-50 text-rose-500 text-[10px] font-black p-3 px-6 rounded-full border-none" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} />
            </div>
                ...
                return (
                  <button
                    ...
                    className={`relative p-8 rounded-[2.5rem] text-[11px] font-black transition-all flex flex-col items-center justify-center border-2 
                      ${isCollisionSlot ? "bg-red-500 text-white border-red-500 shadow-red-200" :
                        isSelected ? "bg-rose-500 text-white border-rose-500 shadow-rose-200" :
                        occupant ? "bg-slate-50 text-slate-300 border-slate-50" : 
                        "bg-white text-slate-400 border-slate-50 hover:border-rose-100 hover:text-rose-400"}
                    `}
                  >
                    <span>{time}</span>
                    {occupant && <span className="absolute bottom-4 text-[7px] uppercase opacity-60">{occupant}</span>}
                  </button>
                );
              })}
            </div>
