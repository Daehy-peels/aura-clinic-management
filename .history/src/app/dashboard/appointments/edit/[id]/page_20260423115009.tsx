      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="animate-in fade-in slide-in-from-left duration-700">
            <Link href="/dashboard/appointments" className="group text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 hover:text-rose-500 transition-all">
              ← Return to Schedule
            </Link>
          ...
          <div className="bg-white border border-slate-100 p-6 px-10 rounded-[2rem] shadow-[0_15px_40px_rgba(0,0,0,0.02)]">
            <p className="text-[9px] font-black text-rose-300 uppercase tracking-widest mb-1">Patient Dossier</p>
        ...
        <form onSubmit={handleUpdate} className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-8">
              ...
              {/* STATUS SELECTOR */}
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-4">Current Status</label>
                  ...
                  {['scheduled', 'completed', 'cancelled'].map((s) => (
                    <button
                      ...
                      className={`py-2 text-[8px] font-black uppercase tracking-tighter rounded-xl transition-all ${
                        formData.status === s ? 'bg-white text-rose-500 shadow-sm' : 'text-slate-400 hover:text-slate-600'
                      }`}
                    >
                      {s}
                    </button>
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2">Treatment Type</label>
                <input
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2">Duration (Minutes)</label>
                <select 
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2">Clinical Notes</label>
                <textarea
                  rows={4}
            ...
            <button type="submit" className="w-full py-6 bg-slate-900 text-white font-black uppercase tracking-[0.3em] text-[10px] rounded-full shadow-xl hover:bg-rose-500 transition-all hover:-translate-y-1 active:scale-95">
              Confirm Amendments
            </button>
          </div>
          ...
          <div className="lg:col-span-8 bg-white p-10 md:p-14 rounded-[4rem] border border-slate-100 shadow-[0_20px_60px_rgba(0,0,0,0.02)]">
            <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
              <h3 className="text-2xl font-light text-slate-800">
                Clinic <span className="italic font-serif text-rose-500">Availability</span>
              </h3>
              <input
                type="date"
