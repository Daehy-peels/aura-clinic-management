...
export default function BookAppointmentPage() {
  ...
  useEffect(() => {
    ...
    if (conflict) {
      setSlotError(
        `Overlap: Slot held by ${conflict.patients?.first_name || "another patient"}.`,
      );
    } else {
      setSlotError(null);
    }
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div>
            <Link
              ...
              className="group flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-rose-300 hover:text-rose-500 transition-colors"
            >
            ...
            <h1 className="text-4xl font-light mt-2">
              Book{" "}
              <span className="font-serif italic text-rose-500 underline decoration-rose-200 underline-offset-4">
                Session
              </span>
            </h1>
          </div>
          ...
          <form
            ...
            className="bg-white p-8 rounded-[2rem] shadow-2xl shadow-rose-100/50 border border-rose-50 space-y-6"
          >
            <div className="space-y-1">
              <label className="text-[10px] font-black text-rose-400 uppercase tracking-[0.2em] ml-1">
                Appointment Date
              </label>
              <input
                type="date"
            ...
            {slotError && (
              <div className="p-4 bg-red-50 border border-red-100 rounded-2xl animate-pulse">
                <p className="text-[11px] text-red-600 font-bold uppercase tracking-tight text-center">
                  ⚠️ {slotError}
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-2 overflow-y-auto max-h-[650px] pr-4 custom-scrollbar">
              {timeSlots.map((slot, i) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-[11px] font-black w-12 ${isOccupied ? "text-red-300" : isSelected ? "text-rose-500" : "text-slate-400"}`}
                    >
                      {slot.time}
                    </span>
