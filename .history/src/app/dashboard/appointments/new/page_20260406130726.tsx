...
export default function BookAppointmentPage() {
  ...
  // 2. CHECK FOR OVERLAPS (Bulletproof Version)
  useEffect(() => {
  if (!formData.scheduled_at || !formData.ended_at || dayAppointments.length === 0) {
    setSlotError(null);
    return;
  }
  ...
  // Find the specific appointment that causes the trouble
  const conflict = dayAppointments.find((apt) => {
    // Only check against 'scheduled' or 'confirmed' appointments
    if (apt.status === "cancelled") return false;

    const existingStart = new Date(apt.scheduled_at).getTime();
    const existingEnd = new Date(apt.ended_at).getTime();

    // Logic: (New appointment starts before old one ends) AND (New ends after old starts)
    const isOverlapping = newStart < existingEnd && newEnd > existingStart;
    return isOverlapping;
  });

  if (conflict) {
    setSlotError(`Overlap! ${conflict.patients?.first_name} is already booked.`);
    console.warn("Conflict Found:", conflict);
  } else {
    setSlotError(null);
  }
}, [formData.scheduled_at, formData.ended_at, dayAppointments]);

// 2. STRENGTHENED SUBMIT HANDLER
const handleSubmit = async (e: React.FormEvent) => {
  e.preventDefault();

  // If the local UI already sees an error, stop immediately
  if (slotError) {
    alert("This slot is already taken. Please choose another time.");
    return;
  }

  if (!formData.patient_id || !formData.treatment_type) {
  ...
  // FINAL SAFETY CHECK: Query Supabase directly one last time
  const { data: finalConflict, error: checkError } = await supabase
    .from("appointments")
    .select("id")
    .neq("status", "cancelled")
  ...
  if (finalConflict) {
    alert("Double booking prevented: This slot was just reserved by another user.");
    setLoading(false);
    // Refresh the page data to show the new appointment in the timeline
    router.refresh(); 
    return;
  }
