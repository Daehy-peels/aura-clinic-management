...
export default function BookAppointmentPage() {
  ...
  useEffect(() => {
    ...
    if (conflict) {
      setSlotError(
        `Time conflict: Already booked by ${conflict.patients?.first_name || "another patient"}.`,
      );
    } else {
      setSlotError(null);
    }
        ...
        <div className="lg:col-span-5">
          <div className="mb-6">
            <Link
              ...
              className="text-xs font-bold text-rose-400 hover:text-rose-600 uppercase tracking-widest"
            >
              ← Back to Dashboard
            </Link>
            <h1 className="text-3xl font-semibold mt-2 text-slate-800">
              New Appointment
            </h1>
          </div>
          ...
          <form
            ...
            className="bg-white border border-slate-100 p-8 rounded-3xl shadow-xl shadow-slate-200/50 space-y-6"
          >
            <div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span
                        className={`text-[10px] font-black ${isOccupied ? "text-red-300" : isSelected ? "text-white" : "text-slate-400"}`}
                      >
                        {slot.time}
                      </span>
                      <div className="text-left">
                        {isOccupied ? (
                          ...
                          <span
                            className={`text-[10px] font-bold uppercase ${isSelected ? "text-white" : "text-emerald-500/60"}`}
                          >
                    ...
                    <div
                      className={`text-[10px] ${isOccupied ? "text-red-400" : isSelected ? "text-white" : "text-emerald-400"}`}
                    >
                      {isOccupied ? "✕" : "✓"}
                    </div>
                  </button>
