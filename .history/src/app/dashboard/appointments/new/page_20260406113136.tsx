...
useEffect(() => {
  ...
  const conflict = dayAppointments.find((apt) => {
    ...
    // Debugging: Uncomment the line below to see comparisons in console
    // console.log(`Checking vs ${apt.patients?.first_name}:`, { newStart, newEnd, existingStart, existingEnd });

    // THE OVERLAP FORMULA
    // An overlap exists if the new appointment starts before the old one ends 
    // AND the new appointment ends after the old one starts.
    return newStart < existingEnd && newEnd > existingStart;
  });

  if (conflict) {
    setSlotError(`Conflict: ${conflict.patients?.first_name} is already booked here.`);
  } else {
    setSlotError(null);
  }
