...
export default function AppointmentsPage() {
  ...
  // Helper to display time without timezone shifts, with added safety checks
  const formatWallTime = (isoString: string) => {
    // 1. Safety Check: If string is missing or empty, return a fallback
    if (!isoString) {
      return { time: "--:--", day: "No Date" };
    }

    try {
      // 2. The split logic we used to bypass timezones
      const localPart = isoString.split(/[+-Z]/)[0];
      const date = new Date(localPart);

      // 3. Check if the resulting date is actually valid
      ...
      console.error("Error formatting date:", isoString, err);
