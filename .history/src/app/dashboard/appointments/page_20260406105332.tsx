...
export default function AppointmentsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error) {
      ...
      fetchAppointments(); // Re-fetch to sync state if it failed
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 space-y-4">
            ...
            <p className="text-rose-300 font-serif italic text-xl">
              Opening the clinical diary...
            </p>
          </div>
          ...
          <div className="space-y-6">
            {filteredAppointments.map((apt) => {
              ...
              return (
                <div
                  key={apt.id}
                  className={`group transition-all duration-700 rounded-[3rem] p-8 border flex flex-col md:flex-row items-center justify-between gap-8 backdrop-blur-sm ${
                    isCompleted
                      ? "bg-emerald-50/40 border-emerald-100/50"
                      : isCancelled
                        ? "bg-gray-100/40 border-gray-200 opacity-60 grayscale-[0.8]"
                        : "bg-white/80 border-white hover:shadow-rose-100 shadow-sm"
                  }`}
                >
                  {/* Time Section */}
                  <div className="flex flex-col items-center justify-center md:border-r md:border-gray-100 md:pr-12 min-w-[140px]">
                    <p
                      className={`text-3xl font-light tracking-tighter ${isCancelled ? "text-gray-400" : "text-gray-800"}`}
                    >
                    ...
                    <p
                      className={`text-[10px] font-black uppercase tracking-[0.2em] mt-2 ${isCompleted ? "text-emerald-400" : "text-rose-300"}`}
                    >
                  ...
                  <div className="flex-1 text-center md:text-left">
                    <h3
                      className={`text-sm font-bold uppercase tracking-widest ${isCancelled ? "text-gray-400 line-through" : "text-gray-700"}`}
                    >
                      {apt.patients?.first_name} {apt.patients?.last_name}
                    </h3>
                    <p
                      className={`text-2xl font-serif italic ${isCompleted ? "text-emerald-700/60" : isCancelled ? "text-gray-400" : "text-rose-500"}`}
                    >
                      {apt.treatment_type}
                    </p>
                  </div>
                  ...
                  {/* Action Group */}
                  <div className="flex items-center gap-3 bg-white/50 p-2 rounded-[2.5rem] shadow-sm border border-white">
                    ...
                    <Link
                      ...
                      className="p-4 text-gray-300 hover:text-rose-500 hover:bg-white rounded-2xl transition-all"
                      title="Edit Details"
                    >
                      <EditIcon />
                    </Link>
