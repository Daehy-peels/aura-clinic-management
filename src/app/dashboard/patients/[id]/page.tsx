        ...
        <div className="flex justify-between items-center mb-12">
          ...
          <div className="flex gap-3">
            <Link
              ...
              className="px-6 py-2 border border-slate-100 rounded-full text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-all"
            >
              Edit Profile
            </Link>
            <Link
              href="/dashboard/appointments/new"
              className="px-6 py-2 bg-rose-500 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg shadow-rose-100 hover:bg-rose-600 transition-all"
            >
              Book Session
            </Link>
          </div>
        ...
        <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)] mb-10 flex flex-col md:flex-row gap-10 items-center">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-2">
              ...
              <span className="px-4 py-1 bg-emerald-50 text-emerald-500 text-[8px] font-black uppercase tracking-[0.2em] rounded-full">
                Validated Patient
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            {activeTab === "overview" && (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100">
                  <h3 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.3em] mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <InfoItem
                      label="Registration"
                      value={format(new Date(patient.created_at), "PPP")}
                      icon={<IconCalendar />}
                    />
                  </div>
                </div>
                ...
                <div className="bg-rose-50/30 rounded-3xl p-8 border border-rose-100/50">
                  <p className="text-slate-600 leading-relaxed font-serif italic text-lg">
                    ...
                    {patient.medical_history ||
                      "The patient's clinical history is currently clear. No contraindications recorded."}
                    "
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                {appointments.length === 0 ? (
                  ...
                  appointments.map((apt) => (
                    <div
                      key={apt.id}
                      className="bg-white p-8 rounded-3xl border border-slate-100 flex items-center justify-between group hover:border-rose-200 transition-all"
                    >
                      <div className="flex gap-8 items-center">
                        <div className="text-center min-w-[60px]">
                          ...
                          <p className="text-[9px] font-black text-rose-400 uppercase">
                            {format(parseISO(apt.scheduled_at), "MMM")}
                          </p>
                        </div>
                      ...
                      <div className="flex items-center gap-6">
                        <span
                          className={`text-[8px] font-black uppercase px-3 py-1 rounded-full ${apt.status === "completed" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                        >
                          {apt.status}
                        </span>
                        <Link
                          href={`/dashboard/appointments/edit/${apt.id}`}
                          className="p-3 bg-slate-50 rounded-full opacity-0 group-hover:opacity-100 transition-all hover:bg-rose-50 text-slate-400 hover:text-rose-500"
                        >
                          <IconEdit />
                        </Link>
                      </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white overflow-hidden relative group">
              ...
              <h4 className="text-[9px] font-black text-rose-400 uppercase tracking-[0.4em] mb-6 relative z-10">
                Care Notes
              </h4>
              <p className="text-sm text-slate-300 leading-relaxed relative z-10">
                Last seen for{" "}
                <span className="text-white font-bold">
                  {appointments[0]?.treatment_type || "Initial Consultation"}
                </span>
              ...
              <div className="mt-8 pt-6 border-t border-white/10 relative z-10">
                ...
                <p className="text-xs italic text-rose-200 mt-1">
                  High-priority patient record
                </p>
              </div>
            </div>

            <div className="bg-rose-50/50 border border-rose-100 rounded-[2.5rem] p-10">
              <h4 className="text-[9px] font-black text-rose-400 uppercase tracking-[0.4em] mb-4">
                Patient Integrity
              </h4>
              ...
              <p className="text-[10px] text-slate-400 leading-tight">
                All clinical data is encrypted and stored according to medical
                privacy standards.
              </p>
            </div>
          </div>
